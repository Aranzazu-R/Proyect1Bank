...
def test_create_transaction():
    ...
    try:
        payment = PaymentController.pay(card=card, amount=20_000, concept="Pago tarjeta",
    ...
    # Act
    created_payment = Payment.select().where(Payment.id == payment.id).get()

    # Assert
    assert payment.id == created_payment.id
    assert payment.card == created_payment.card
    assert payment.amount == created_payment.amount
    assert payment.concept == created_payment.concept
    assert payment.payment_date == created_payment.payment_date

    # Delete test instances
    card.delete_instance()
    user.delete_instance()
    created_transaction.delete_instance()
    created_payment.delete_instance()
